...
### Step 3(第二部分): Python运行端口显示###
方法先通过接收udp帧并将字节流转换为16进制字符串，并将每一通道的2字节，如201C，先将20和1C翻转，并扩展为“1C200000”的四字节字符串，最后再转为四字节浮点数。


设置程序权限：`chmod 777 send` 
执行发送：`./send`

运行显示程序`python3 robot_server.py`
